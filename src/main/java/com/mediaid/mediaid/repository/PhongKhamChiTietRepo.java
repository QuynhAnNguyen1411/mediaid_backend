...
public interface PhongKhamChiTietRepo extends JpaRepository<PhongKhamChiTiet, String> {
    ...
    @Query(nativeQuery = true, value = "select a.* " +
            ...
            "where c.phan_vungid = ?1; ")
    List<PhongKhamChiTiet> findbyPhanVungID(String phanVungID);
    ...
    @Query(nativeQuery = true, value = "select a.* " +
            ...
            "where c.bo_phanid = ?1;")
    List<PhongKhamChiTiet> findbyBoPhanID(String boPhanID);

    @Query(nativeQuery = true, value = "select a.* " +
            "from mediate_db.phong_kham_chi_tiet a " +
            "inner join mediate_db.phong_kham b on a.phong_khamid = b.phong_khamid " +
            "inner join mediate_db.chan_doan_bang_bo_phan c on b.phong_khamid = c.phong_khamid " +
            "where c.bo_phanid = ?1 AND c.phong_kham_ho_tro = ?2;")
    List<PhongKhamChiTiet> findPhongKhamHoTrobyBoPhanID(String boPhanID, boolean isPhongKhamHoTro);

    @Query(nativeQuery = true, value = "select a.* " +
            "from mediate_db.phong_kham_chi_tiet a " +
            "where a.phong_khamid = ?1;")
    List<PhongKhamChiTiet> findByphonngKhamID(String phonngKhamID);
}
